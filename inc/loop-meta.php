...
<?php } elseif ( is_search() ) { ?>
	...
	<div class="loop-description search-description">
		<?php echo wpautop( sprintf( __( 'You are browsing the search results for "%s"', 'piquant' ), esc_attr( get_search_query() ) ) ); ?>
	</div>
...
<?php } elseif ( is_day() || is_month() || is_year() ) { ?>
	...
	<?php
		if ( is_day() )
			$date = get_the_time( __( 'F d, Y', 'piquant' ) );
		elseif ( is_month() )
			$date = get_the_time( __( 'F Y', 'piquant' ) );
		elseif ( is_year() )
			$date = get_the_time( __( 'Y', 'piquant' ) );
	?>

	<h1 class="page-title date-title"><span><?php echo $date; ?></span></h1>

	<div class="loop-description date-description">
		<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'piquant' ), $date ) ); ?>
	</div>

<?php } elseif ( is_archive() ) { ?>

	<h1 class="page-title archive-title"><span><?php _e( 'Archives', 'piquant' ); ?></span></h1>

	<div class="loop-description archive-description">
		<?php echo wpautop( __( 'You are browsing the site archives.', 'piquant' ) ); ?>
	</div>
